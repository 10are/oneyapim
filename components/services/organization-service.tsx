...
export default function OrganizationServicePanel() {
  ...
  return (
    <motion.section
      ...
      className="relative overflow-hidden rounded-3xl border border-purple-600/50 bg-gradient-to-br from-[#18081f] via-[#1a0e22] to-[#2a0f17] shadow-2xl"
    >
      {/* Neon stage lights */}
      <div className="absolute inset-0 pointer-events-none" aria-hidden>
        <div className="absolute -top-24 -left-10 h-64 w-64 rounded-full bg-fuchsia-500 blur-[100px] opacity-30" />
        <div className="absolute -top-16 right-0 h-72 w-72 rounded-full bg-amber-400 blur-[120px] opacity-25" />
        <div className="absolute bottom-[-4rem] left-1/3 h-72 w-72 rounded-full bg-red-500 blur-[120px] opacity-20" />
      </div>

      {/* Title bar */}
      <div className="relative z-10 flex items-center gap-3 px-5 sm:px-6 py-3 border-b border-purple-700/40 bg-black/30 backdrop-blur">
        ...
        <div className="flex items-center gap-2 text-slate-200/90 text-sm">
          ...
          <span className="truncate max-w-[22ch] text-slate-300/80">{t?.services?.organization?.description ?? 'Planlama, prodüksiyon ve yönetim'}</span>
        </div>
        <button className="ml-auto text-xs text-slate-300/90 rounded-md border border-white/10 px-2 py-1 hover:bg-white/5 transition">Yeni Etkinlik</button>
      </div>

      {/* Shell */}
      <div className="relative z-10 grid grid-cols-[64px_1fr] xl:grid-cols-[80px_1fr] min-h-[680px]">
        {/* Left rail */}
        <nav className="flex flex-col items-center gap-5 py-4 border-r border-purple-700/40 bg-black/30">
          {[
            ...
            <button
              ...
              className={`group p-2.5 rounded-xl transition focus:outline-none focus:ring-2 focus:ring-fuchsia-500/40 ${
                active
                  ? 'bg-gradient-to-b from-purple-700/40 to-purple-900/40 text-fuchsia-300'
        ...
        <div className="flex flex-col min-w-0">
          {/* Breadcrumbs / filters */}
          <div className="flex items-center gap-2 px-4 py-2 text-[12px] text-slate-300/80 border-b border-purple-700/40 bg-black/20">
            <span className="text-slate-500">Etkinlikler</span>
            <ChevronRight className="h-3 w-3" />
            <span className="text-slate-200">Haftalık Plan</span>
          ...
          <div className="grid lg:grid-cols-3 gap-6 p-5">
            ...
            <motion.div
              ...
              className="lg:col-span-2 rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-5"
            >
              <div className="flex items-center gap-2 mb-4">
                <Clock3 className="h-5 w-5 text-amber-300" />
                <h4 className="text-slate-100 font-semibold">Sahne Zaman Çizelgesi</h4>
              </div>
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-slate-100 font-semibold">Bilet Satışı</h4>
              </div>
              <div className="h-32">
          ...
          <div className="border-t border-purple-700/40 bg-black/30">
            <div className="flex items-center gap-3 px-4 py-2 text-[12px] text-slate-300">
              ...
              <div className="ml-auto flex items-center gap-4 text-slate-400">
                ...
                <span>Giriş yoğunluğu bekleniyor (19:00)</span>
              </div>
            </div>
            <div className="px-4 py-3 font-mono text-[12.5px] leading-6 text-slate-200">
              <div className="text-emerald-400"># QR okutuldu → Sıra 10234</div>
              <div className="text-slate-400">✓ Erkenden gelenler için ayrı hat açıldı</div>
              <div className="text-emerald-400"># QR okutuldu → Sıra 10235</div>
              <div className="text-slate-400">✓ Güvenlik kontrolü hızlı</div>
            </div>
          </div>
        </div>
      </div>
...
function TimelineItem({ time, title, icon: Icon, status, color, pulse }: { time: string; title: string; icon: any; status: string; color?: string; pulse?: boolean }) {
  return (
    <div className="grid grid-cols-[72px_1fr_auto] items-center gap-3">
      <div className="text-slate-400 text-sm tabular-nums">{time}</div>
      <div className="flex items-center gap-3">
        <Icon className={`h-5 w-5 ${color ?? 'text-slate-300'}`} />
        <div>
          <div className="text-slate-100 font-medium">{title}</div>
          <div className="text-slate-400 text-xs">Ana sahne</div>
        </div>
      </div>
      <div className="flex items-center gap-2 text-xs">
        <span className={`px-2 py-0.5 rounded-full border border-white/10 bg-white/5 ${pulse ? 'animate-pulse' : ''}`}>{status}</span>
      </div>
    </div>
  );
}
...
function InfoCard({ title, icon: Icon, accent, items }: { title: string; icon: any; accent: string; items: { k: string; v: string }[] }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-5 relative overflow-hidden">
        ...
        <ul className="space-y-1.5 text-sm">
          {items.map((it) => (
            <li key={it.k} className="flex items-center justify-between">
              <span className="text-slate-300/90">{it.k}</span>
              <span className="text-slate-100 font-medium">{it.v}</span>
            </li>
          ))}
        </ul>
      </div>
